...
- 检测流断线每 5 秒重连；高清 RTSP 读取超时为 15 秒，录制进程退出会重启；另每 20 秒检查文件写入，连续 30 秒无进展也会重启录制（不只检查 PID 是否存在）。
- 剪辑前检查真实媒体时长与分段连续性，录像未覆盖事件就拒绝剪辑，不拿旧录像顶替。剪辑后及发送前必须确认存在至少 1 秒的视频流且首帧可解码；FFmpeg 返回 0、文件非空并不代表视频有效。
- 推送失败只记日志，片段留在 `OUTPUT_DIR`，
  推送成功则删掉本地副本。
...
- 断流期间没录下来的活动无法靠重连补回。时间覆盖校验允许文件名秒级精度带来的 1 秒误差；有效性校验解码首帧，不是整段逐帧完整性检查。
