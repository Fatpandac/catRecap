...
class ActivityTracker:
    ...
    _last_motion_at: float | None = field(default=None, repr=False)
    ...
    def update(self, timestamp: float, centers: list[tuple[float, float]]) -> str | None:
        ...
        moving = (
            ...
            and self._last_centroid is not None
            and dist(centroid, self._last_centroid) > self.move_threshold
        )
        ...
        if self.active:
            ...
            if (
                timestamp - self._last_motion_at > self.post_seconds
        ...
        if not moving:
            return None
        if self._stopped_at is not None and timestamp - self._stopped_at < self.cooldown:
            return None
        self.active = True
